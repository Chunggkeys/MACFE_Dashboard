...
def voltage_test(voltage,pwm_pin):
    ...
    if duty_cycle>=0 and duty_cycle<=100: #checks to see if the duty cycle is valid 
        global output
        output.ChangeDutyCycle(duty_cycle) #changes the duty cycle to output the voltage required
...
def monitor (pins, debounce):
    print("----------------------------------------------------------------------------")
    file=open("data.txt","w") #opens the file for writing signal data, used for graph
    ...
    for i in range(length): #gets the state of the pins (HIGH/LOW)
        ...
        if debounce==True: #if the user wants to debounce, it's done using another function 
            debounces(pins[i])
    ...
    with keyboard.Listener(on_press=on_press) as listener:
        ...
        while break_program==False: #runs until a signal comes to all the pins
            ...
            for i in range (length):#gets the current state of the pins 
                ...
                if input_value[i]!=compare_value[i]: #detects the signal by comparing the current state to previous state
                    t1=time.time()-t0 #gets the time at when the signal was detected

                    if compare_value[i]==IO.HIGH:
                        print("Signal Detected for pin",pins[i]," at ",round(t1,2)," seconds")
                        IO.output(led_pin,1) #lights up the LED to show that a signal came
                    else:
                        print("Signal Lost for pin",pins[i]," at ",round(t1,2)," seconds")
                    
                    input_value[i]=compare_value[i] #the latest state of the pin is stored in input_value
                    
                    file.write("%s %s %s\n" % (t1,pins[i],compare_value[i])) #writes the data to the file
...
def main():
    ...
    while run==True: #runs until user exits
        ...
        print("----------------------------------------------------------------------------") #just for organizing
        print ("1. Voltage Test\n2. Push button\n3. Periodic Signal\n4. Monitor Pins\n5. Timing Diagrams\n6. End Program") #user options
        print("----------------------------------------------------------------------------")
        option=int(input("Enter the option number ")) #gets the user inputs
        
        while option>6 or option<1:
            print("----------------------------------------------------------------------------")
            option=int(input("Invalid option: Enter the option number again")) #handles invalid option
            
        if option==1: #runs the voltage test function
            print("----------------------------------------------------------------------------")
                
            voltage=float(input("Enter the voltage that you want to output ")) #gets voltage 
            pin=int(input("Enter the pwm pin you want to use ")) #gets the pwm pin
            
            while(voltage<0 or voltage>5) or (pin!=23 and pin!=26 and pin!=1): #handles exceptions 
                print("----------------------------------------------------------------------------")
            ...
            output=IO.PWM(pin,100) #sets the PWM to 100 frequency to avoid flickering 
            ...
            voltage_button=float(input("Enter the voltage that you want to output ")) #gets the voltage 
            pin_button=int(input("Enter the pwm pin you want to use ")) #gets the pwm pin
            push_button=int(input("Enter the pin for push button ")) #gets the pin for push button 

            while (voltage_button<0 or voltage_button>5) or (pin_button!=23 and pin_button!=26 and pin_button!=1) or (push_button<0): #handles exceptions 
                print("----------------------------------------------------------------------------")
                print("Invalid Input, Enter the Information Again")
                voltage_button=float(input("Enter the voltage that you want to output "))
            ...
            push_voltage(voltage_button,pin_button,push_button) #calls the push_voltage function 

            
            
        elif option==3:#runs the periodic_signal function 
            print("----------------------------------------------------------------------------")
            pin_period=int(input("Enter the pin for periodic signals ")) #gets the pin for the signal
            period=float(input("Enter the period between signals in milliseconds ")) #gets the period for signal 

            while (pin_period<0) or (period<0): #handles the exceptions 
                print("----------------------------------------------------------------------------")
                print("Invalid Input, Enter the information again ")
                pin_period=int(input("Enter the pin for periodic signals "))
                period=float(input("Enter the period between signals in milliseconds "))
            ...
            print("----------------------------------------------------------------------------")
            length=int(input("Enter the number of pins you want to monitor ")) #gets the amount of pins
            ...
            debouncing=int(input("Do you want to debounce the pins (Enter 1 for YES, 2 for NO) ")) #asks for debounce
